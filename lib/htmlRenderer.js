    ...
    return employeeRoster.map(employee => {
        ...
        <div class="card" style="max-width: 18rem;">
            <div class="card-header bg-primary mb-3 text-white">
                <h5 class="card-title">${employee.getTitle()}</h5>
                <h5 class="card-title">${employee.getName()}</h5>
            </div>
            <div class="card-body text-black">
                <ul class="list-group">
                    <li class="list-group-item">ID:${employee.getId()}</li>
                    <li class="list-group-item">Email:${employee.getEmail()}</a></li>
                </ul>
            </div>
        </div>
        `
    ...
    <!doctype html>
        ...
        <body>
            <nav class="navbar navbar-dark bg-dark">
                <a class="navbar-brand mx-auto" href="main.html">Meet Our Team</a>
            </nav>
            <div class="container">
                ${employeeCards}
            </div>
...
function createHTML (employeeRoster){
    const employeeCards = generateCards(employeeRoster);
    const html = employeePage(employeeCards);

    fs.writeFile(path.join(_dirname, '..', 'output', 'index.html'), html, err =>{
        if (err) throw err;
        console.log('SUCCESS!');
    });
};
